...
namespace SubscriptionManager.Application.Tests.DigitalServices.DeleteDigitalService;

public sealed class DeleteDigitalServiceHandlerTests
{
    [Fact]
    public async Task HandleAsync_ShouldDeleteCurrentUserCustomDigitalService_WhenDigitalServiceExists()
    {
        ...
        var handler = new DeleteDigitalServiceHandler(
            repository.Object,
            currentUser.Object);

        var result = await handler.HandleAsync(
            new DeleteDigitalServiceCommand(digitalService.Id));

        Assert.True(result);

        repository.Verify(
            x => x.Remove(digitalService),
            Times.Once);
    ...
    public async Task HandleAsync_ShouldReturnFalse_WhenCurrentUserCustomDigitalServiceDoesNotExist()
    {
        ...
        var result = await handler.HandleAsync(
            new DeleteDigitalServiceCommand(digitalServiceId));

        Assert.False(result);

        repository.Verify(
            x => x.Remove(It.IsAny<DigitalService>()),
            Times.Never);
